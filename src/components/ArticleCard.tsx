...
export default function ArticleCard({ article, featured = false }: ArticleCardProps) {
  ...
  if (featured) {
    return (
      ...
      >
        <div className="relative h-full bg-zinc-900 rounded-xl p-6">
          <div className="absolute top-4 right-4 flex items-center gap-2">
            <span className="px-2 py-1 text-xs font-medium bg-emerald-500/10 text-emerald-400 rounded-full">
              Featured
            </span>
          </div>
          <h3 className="text-xl font-bold text-zinc-100 mt-2 group-hover:text-emerald-400 transition-colors pr-20">
            {article.title}
          </h3>
          {article.excerpt && (
  ...
  return (
    <Link
      to={`/${article.slug}`}
      className="group card p-5 hover:border-emerald-500/40"
    >
