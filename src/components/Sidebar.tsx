...
export default function Sidebar({ onToggle }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-700 border-r border-slate-600">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-600 flex-shrink-0">
        ...
        <button
          onClick={onToggle}
          className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-slate-600 transition-colors flex-shrink-0"
        ...
        {tracks.map((track) => (
          <div key={track.id} className="pt-3">
            <button
              ...
              className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg border ${trackBorder[track.color]} bg-slate-600 hover:bg-slate-500 transition-colors mb-1`}
            >
