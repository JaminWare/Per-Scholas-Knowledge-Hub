    ...
    <div className="space-y-6 text-zinc-300 leading-relaxed">
      {blocks.map((block, i) => {
        switch (block.type) {
          case 'intro':
            return (
              <p key={i} className="text-base text-zinc-300 leading-relaxed border-l-4 border-emerald-500 pl-4 py-1 italic">
                {block.text}
              </p>
            );
          ...
          case 'tip':
            return (
              <div key={i} className="flex gap-3 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                <Lightbulb className="w-5 h-5 text-emerald-400 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-emerald-300">{block.text}</p>
              </div>
            );
            ...
            return (
              <div key={i} className="rounded-xl overflow-hidden border border-zinc-700">
                ...
                <pre className="p-4 bg-zinc-950 overflow-x-auto text-sm">
                  <code className="text-emerald-300 font-mono leading-relaxed whitespace-pre">{block.code}</code>
                </pre>
              </div>
            );
