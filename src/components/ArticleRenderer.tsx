    ...
    <div className="space-y-6 text-zinc-600 dark:text-zinc-300 leading-relaxed">
      {blocks.map((block, i) => {
        switch (block.type) {
          case 'intro':
            return (
              <p key={i} className="text-base text-zinc-600 dark:text-zinc-300 leading-relaxed border-l-4 border-sky-500 pl-4 py-1 italic">
                {block.text}
              </p>
            );
          ...
          case 'heading':
            return (
              <h2 key={i} className="text-xl font-bold text-zinc-800 dark:text-zinc-100 mt-8 pt-4 border-t border-zinc-200 dark:border-zinc-800 first-of-type:border-0 first-of-type:mt-4">
                {block.text}
              </h2>
            );
          ...
          case 'paragraph':
            return (
              <p key={i} className="text-zinc-600 dark:text-zinc-400">{block.text}</p>
            );

          case 'warning':
            return (
              <div key={i} className="flex gap-3 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20">
          ...
          case 'tip':
            return (
              <div key={i} className="flex gap-3 p-4 rounded-xl bg-sky-50 dark:bg-sky-500/5 border border-sky-200 dark:border-sky-500/20">
                <Lightbulb className="w-5 h-5 text-sky-600 dark:text-sky-400 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-sky-700 dark:text-sky-300">{block.text}</p>
              </div>
            );
            ...
            return (
              <ol key={i} className="space-y-2">
                {block.items.map((step, j) => (
                  <li key={j} className="flex gap-3 text-sm text-zinc-600 dark:text-zinc-300">
                    <ChevronRight className="w-4 h-4 text-sky-500 dark:text-sky-400 flex-shrink-0 mt-0.5" />
                    <span>{step}</span>
                  </li>
                ))}
              </ol>
